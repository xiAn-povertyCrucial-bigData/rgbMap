     ...
     var str =`
       <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">乡（镇）内务工</td>						
					<td colspan="2">乡（镇）外县内务工</td>		
                    <td colspan="2">县外省内务工</td>						
					<td colspan="2">省外务工</td>					
				</tr>		
                 <tr >						
					<td>人数</td>
					<td>占比（%）</td>
		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].B, name:'乡（镇）内务工'},
                          {value:data.data.list[n-1].C, name:'乡（镇）外县内务工'},
                          {value:data.data.list[n-1].D, name:'县外省内务工'},
                          {value:data.data.list[n-1].E, name:'省外务工'}
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].B, data.data.list[n-1].C, data.data.list[n-1].D, data.data.list[n-1].E]
                  }
              ],
			 }
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,six,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "povStatExcel/export/exist/workstatestat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/workstatestat?"+b;
			console.log(cc);
