     ...
     var str =`
          <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td rowspan="2">贫困人口数</td>						
					<td colspan="2">一般贫困户人数</td>						
					<td colspan="2">低保贫困户人数</td>						
					<td colspan="2">五保贫困户人数</td>						
					<td colspan="2">一般贫困户数</td>						
					<td colspan="2">低保贫困户数</td>	
                    <td colspan="2">五保贫困户数</td>						
				</tr>					
				<tr >						
					<td>人数</td>
					<td>占比（%）</td>
		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].D, name:'一般贫困户数'},
                          {value:data.data.list[n-1].E, name:'低保贫困户数'},
                          {value:data.data.list[n-1].F, name:'五保贫困户数'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].A,data.data.list[n-1].B, data.data.list[n-1].C]
                  }
              ],
			 }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdproper",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdproper?"+b;
			console.log(cc);
