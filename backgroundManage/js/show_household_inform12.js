  ...
  function   showDevelopment(index,obj,data){
    ...
    var str =`
    	...
    	<tr rowspan="2">
					...
					<td rowspan="2">平均年龄</td>						
					<td colspan="2">16岁以下</td>						
					<td colspan="2">16-30岁</td>						
					<td colspan="2">30-40岁</td>	
            <td colspan="2">40-50岁</td>	
            <td colspan="2">50-60岁</td>	
        ...
        for(let i = 0;i < data.data.list.length;i++)
		{
			str +=`<tr>
					...
					<td>${data.data.list[i].AVG_AGE_NUM}</td>
		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].A, name:'16岁以下'},
                          {value:data.data.list[n-1].B, name:'16-30岁'},
                          {value:data.data.list[n-1].C, name:'30-40岁'},
                          {value:data.data.list[n-1].D, name:'40-50岁'},
						  {value:data.data.list[n-1].E, name:'50-60岁'},
	            ...
	            if(data.code==2000)
					{
                     ...
                     $("#total").html("总数:"+data.data.total);
			            ...
			            <td colspan="2">60岁以上</td>																
							...
							<tr><td colspan='20' style="font-size:30px;color:#767272;">查无数据</td></tr>`
							two.html(str)
							$("#total").empty();
						    $(".allPage").empty();
						}else if(data.code==1005){
...
$("body").on("click","#dataImport",function(){
	$.ajax({
        url: url + "povStatExcel/export/exist/agegroupstat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/agegroupstat?"+b;
			console.log(cc);
