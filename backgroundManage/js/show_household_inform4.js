   ...
   //  var n=0;
     ...
     <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
                    ...
                    <td  valign="middle">脱贫不享受政策户数</td>
                    <td  valign="middle">脱贫不享受政策人数</td>				
               </tr>					
                    `;
...
$("body").on("click","#selectCity span",function(){
    	...
    	$(".active1").each(function(){
	      ...
	      if( city == 'countyName'){
					 data ={ "page_index":1,"page_size": 50,'aar040':k[0],"aar001":code,"aac005":k[1]}
			}else if( city == "townName"){
					 data ={ "page_index":1,"page_size": 50,'aar040':k[0],"aar002":code,"aac005":k[1] }					
			    }else if(city == "villageName" ){
				 data = { "page_index":1,"page_size": 50,'aar040':k[0],"aar003":code,"aac005":k[1]}
		              }else{
					   data = { "page_index":1,"page_size": 50,'aar040':k[0],"aac005":k[1] }
					  }
	   $.ajax({
...
$("body").on("click","#dataImport",function(){
	$.ajax({
    url: url + "povStat/export/exist/stablePov", 
    data:{},
    beforeSend:function(request){
        request.setRequestHeader("Authorization",token);
        request.setRequestHeader("X-Requested-With","XMLHttpRequest");
    	...
    	$(".active1").each(function(){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStat/export/naturalChange?"+b;
			console.log(cc);
