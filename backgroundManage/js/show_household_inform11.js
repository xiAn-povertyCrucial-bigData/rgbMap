   ...
   //  var n=0;
     ...
     <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">女性</td>						
					<td colspan="2">少数民族</td>						
					<td colspan="2">持证残疾</td>						
					<td colspan="2">现役军人</td>						
					<td colspan="2">享受低保</td>						
					<td colspan="2">参加大病保险</td>						
				</tr>					
				<tr >						
					<td>人数</td>
					<td>占比（%）</td>
            ...
            success: function(data){
	            if(data.code==2000){
                     ...
                     $("#total").html("总数:"+data.data.total);
							...
							<tr><td colspan='18' style="font-size:30px;color:#767272;">查无数据</td></tr>`
							one.html(str)
							$("#total").empty();
						    $(".allPage").empty();
						}else if(data.code==1005){
...
$("body").on("click","#dataImport",function(){
	$.ajax({
        url: url + "povStatExcel/export/exist/perbasicstat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
	      ...
	      if( dataType == 'aar001'){
					 data ={ "page_index":indexs,"page_size": 50,'aar040':k[0],"aar001":codeType,"aac005":k[1],"aar010":k[2] }
			}else if( dataType == "aar002"){
					 data ={ "page_index":indexs,"page_size": 50,'aar040':k[0],"aar002":codeType,"aac005":k[1],"aar010":k[2] }					
			    }else if(dataType == "aar003" ){
				 data = { "page_index":indexs,"page_size": 50,'aar040':k[0],"aar003":codeType,"aac005":k[1],"aar010":k[2] }
		              }else{
					   data = { "page_index":indexs,"page_size": 50,'aar040':k[0],"aac005":k[1],"aar010":k[2] }
					  }
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/perbasicstat?"+b;
			console.log(cc);
