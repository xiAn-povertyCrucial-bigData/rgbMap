  ...
  let token=$.cookie("token");
	...
	$.ajax({
		...
		success: function(data){
				if(data.code == 2000){
					showDevelopment(0,sixteen,data);
  ...
  function   showDevelopment(index,obj,data){
    ...
    <table border="" cellspacing="" cellpadding="" style="width:1400px;overflow-x:auto">		   
	    		<tr rowspan="2">
					...
					<td rowspan="2" valign="middle">平均住房面积</td>
					<td colspan="2">0平米户数</td>						
					<td colspan="2">10平米以内户数</td>						
					<td colspan="2">10-25平米户数</td>		
                    <td colspan="2">25-50平米户数</td>						
					<td colspan="2">50-75平米户数</td>
                    <td colspan="2">75-100平米户数</td>						
					<td colspan="2">100平米以上户数</td>									
				</tr>		
                 <tr >						
					<td>户数</td>
					<td>占比（%）</td>
        ...
        for(let i = 0;i < data.data.list.length;i++)
		{
			str +=`<tr>
					...
					<td>${data.data.list[i].AVG_HOUSE}</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'0平方米户数'},
                          {value:data.data.list[n-1].B, name:'10平方米以内户数'},
                          {value:data.data.list[n-1].C, name:'10-25平方米户数'},
                          {value:data.data.list[n-1].D, name:'25-50平方米户数'},
						  {value:data.data.list[n-1].E, name:'50-75平方米户数'},
                          {value:data.data.list[n-1].F, name:'75-100平方米户数'},
						  {value:data.data.list[n-1].G,name:'100平米以上户数'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,sixteen,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/avghouse",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/avghouse?"+b;
			console.log(cc);
