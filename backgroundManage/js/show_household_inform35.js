  ...
  let token=$.cookie("token");
	...
	$.ajax({
		...
		success: function(data){
				if(data.code == 2000){
					showDevelopment(0,fifteen,data);
  ...
  function   showDevelopment(index,obj,data){
    ...
    <table border="" cellspacing="" cellpadding="">		   
	    		 <tr>	
					...
					<td rowspan="2" valign="middle">贫困人口数</td>						
					<td colspan="2">中共党员(人)</td>						
					<td colspan="2">中共预备党员</td>		
                    <td colspan="2">共青团员</td>						
					<td colspan="2">无党派民主人士</td>
	    	   	</tr>
                 <tr>						
					<td>人数</td>
					<td>占比（%）</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'中共党员'},
                          {value:data.data.list[n-1].B, name:'中共预备党员'},
                          {value:data.data.list[n-1].C, name:'共青团员'},
						  {value:data.data.list[n-1].D, name:'无党派民主人士'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,fifteen,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdparty",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdparty?"+b;
			console.log(cc);
