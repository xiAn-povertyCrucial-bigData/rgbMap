	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'落实帮扶责任人户数'},
                          {value:data.data.list[n-1].B, name:'未落实帮扶责任人户数'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].A,data.data.list[n-1].B]
                  }
              ],
			 }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdhelper",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
