   ...
   for(let i = 0; i < data.data.list.length; i++ ){
    var str1=`
     <tr>
          ...
          <td>
              <a href="showDetailsPoor1.html?value=${data.data.list[i].AAC001}&time=${$('#p_year').val()}&address=${data.data.list[i].ADDRESS}">详情</a>
              <a href="revisePoorPerson.html?value=${data.data.list[i].AAC001}&time=${$('#p_year').val()}&address=${data.data.list[i].ADDRESS}">修改</a>
...
$("body").on("click","#dataImport",function(){
   ...
   if($("#home").css("display")=="block"){
	...
	$.ajax({
        url: url + "pkh/export/exist/query",
        data:JSON.stringify(data_),
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
