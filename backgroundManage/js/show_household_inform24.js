$(document).ready(function(){
  ...
  let token=$.cookie("token");
	...
	$.ajax({
		url: url + "povStat/holdmainreason",
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].A,data.data.list[n-1].B, data.data.list[n-1].C, data.data.list[n-1].D,data.data.list[n-1].E,data.data.list[n-1].F,data.data.list[n-1].G,data.data.list[n-1].H, data.data.list[n-1].I, data.data.list[n-1].J,data.data.list[n-1].K,data.data.list[n-1].L]
                  }
              ],
			 }
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,four,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdmainreason",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdmainreason?"+b;
			console.log(cc);
