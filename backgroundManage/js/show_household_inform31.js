     ...
     var str =`
        <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td rowspan="2" valign="moddle">距离主干路 </td>						
					<td colspan="2">普通泥土路</td>						
					<td colspan="2">砂石公路</td>	
                    <td colspan="2">水泥路面公路</td>						
					<td colspan="2">沥青路面公路</td>															
				</tr>		
                 <tr >						
					<td>户数</td>
					<td>占比（%）</td>
        ...
        for(let i = 0;i < data.data.list.length;i++)
		{
			str +=`<tr>
					...
					<td>${data.data.list[i].TOTAL_ROAD}</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].A, name:'普通泥土路'},
                          {value:data.data.list[n-1].B, name:'砂石公路'},
                          {value:data.data.list[n-1].C, name:'水泥路面公路'},
                          {value:data.data.list[n-1].D, name:'沥青路面公路'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,eleven,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdtoroad",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdtoroad?"+b;
			console.log(cc);
