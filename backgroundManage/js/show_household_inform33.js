$(document).ready(function(){
  ...
  let token=$.cookie("token");
	...
	$.ajax({
		url: url + "povStat/holdshiftincome",
		...
		success: function(data){
				if(data.code == 2000){
					showDevelopment(0,thirteen,data);
  ...
  function   showDevelopment(index,obj,data){
     ...
     <table border="" cellspacing="" cellpadding="" style="width:1400px;overflow-x:auto">		   
					...
					<td rowspan="2" valign="middle">户均转移性收入（元） </td>						
					<td colspan="2" valign="middle">户均领取计划生育补贴金额（元）</td>						
					<td colspan="2" valign="middle">户均领取低保金（元）</td>	
                    <td colspan="2" valign="middle">户均领取养老金（元）</td>						
					<td colspan="2" valign="middle">户均领取生态补偿金（元）</td>																					
				</tr>		
                 <tr >						
					<td>金额</td>
					<td>占比（%）</td>
		...
		option={
	    	   series : [
                  {
                      name: '贫困户上年度转移性收入构成情况',
                      type: 'pie',
                      radius : '55%',
                      center: ['50%', '60%'],
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'户均领取计划生育补贴金'},
                          {value:data.data.list[n-1].B, name:'户均领取低保金'},
                          {value:data.data.list[n-1].C, name:'户均领取养老金'},
                          {value:data.data.list[n-1].D, name:'户均领取生态补偿金'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,thirteen,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdshiftincome",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdshiftincome?"+b;
			console.log(cc);
