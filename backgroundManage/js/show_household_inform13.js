		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].A, name:'健康'},
                          {value:data.data.list[n-1].B, name:'长期慢性病'},
                          {value:data.data.list[n-1].C, name:'大病'},
                          {value:data.data.list[n-1].D, name:'残疾'}
	            ...
	            if(data.code==2000)
					{
                     ...
                     $("#total").html("总数:"+data.data.total);
			          ...
			          <td colspan="2">残疾</td>						
							...
							<tr><td colspan='14' style="font-size:30px;color:#767272;">查无数据</td></tr>`
							three.html(str)
							$("#total").empty();
						    $(".allPage").empty();
						}else if(data.code==1005){
...
$("body").on('click',"#content  div table tr:nth-child(n+3) td:nth-child(2)",function(){
	   ...
	   var i = 0;
        ...
        if(codeType.substring(9) == '000'){
		    ...
		    //$("#selectCity span:last-child").css("color","#FFAC37")//.siblings("span").css("color","green");
          //  $("#selectCity span:last")
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "povStatExcel/export/exist/bodystat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/bodystat?"+b;
			console.log(cc);
