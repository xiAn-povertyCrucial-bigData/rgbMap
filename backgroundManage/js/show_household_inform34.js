  ...
  let token=$.cookie("token");
	...
	$.ajax({
		...
		success: function(data){
				if(data.code == 2000){
					showDevelopment(0,fourteen,data);
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'一人户'},
                          {value:data.data.list[n-1].B, name:'二人户'},
                          {value:data.data.list[n-1].C, name:'三人户'},
                          {value:data.data.list[n-1].D, name:'四人户'},
						  {value:data.data.list[n-1].E, name:'五人户'},
                          {value:data.data.list[n-1].F, name:'六人户以上'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,fourteen,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdperscale",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdperscale?"+b;
			console.log(cc);
