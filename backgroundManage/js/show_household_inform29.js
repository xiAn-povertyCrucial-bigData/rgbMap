     ...
     var str =`
           <table border="" cellspacing="" cellpadding="" style="width:1600px;overflow-x:auto">		   
	    		<tr rowspan="2">
					...
					<td rowspan="2" valign="middle">人均纯收入(元) </td>						
					<td colspan="2">500元（不含）以下户数</td>						
					<td colspan="2">500-1000元户数</td>
                    <td colspan="2">1000-1500元户数</td>						
					<td colspan="2">1500-2000元户数</td>
                    <td colspan="2">2000-2952元户数</td>
                    <td colspan="2">2952元以上户数</td>						
					<td colspan="2">3100元以上户数</td>												
				</tr>		
                 <tr >						
					<td>户数</td>
					<td>占比（%）</td>
        ...
        for(let i = 0;i < data.data.list.length;i++)
		{
			str +=`<tr>
					...
					<td>${data.data.list[i].AVG_INCOME}</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'500元（不含）以下'},
                          {value:data.data.list[n-1].B, name:'500-1000元'},
                          {value:data.data.list[n-1].C, name:'1000-1500元'},
                          {value:data.data.list[n-1].D, name:'1500-2000元'},
						  {value:data.data.list[n-1].E, name:'2000-2952元'},
                          {value:data.data.list[n-1].F, name:'2952元以上'},
						  {value:data.data.list[n-1].G,name:'3100元以上'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].A,data.data.list[n-1].B, data.data.list[n-1].C, data.data.list[n-1].D,data.data.list[n-1].E,data.data.list[n-1].F,data.data.list[n-1].G]
                  }
              ],
			 }
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,nine,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdlastper",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdlastper?"+b;
			console.log(cc);
