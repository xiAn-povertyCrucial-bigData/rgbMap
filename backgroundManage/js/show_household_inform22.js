$(document).ready(function(){
  ...
  let token=$.cookie("token");
	...
	var data ;
	var dataType;
	var pieElsePoverty = $("#pieElsePoverty");
	var barElsePoverty = $("#barElsePoverty");
  ...
  function   showDevelopment(index,obj,data){
     ...
     var n=0;
     var str =`
        <table class="two1" border="" cellspacing="" cellpadding="" style="width: 2000px;overflow-x:auto;">		   
	    		<tr >
		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].PROINCOME, name:'工资性收入户均（元）'},
                          {value:data.data.list[n-1].WORKINCOME, name:'财产性收入户均（元）'},
                          {value:data.data.list[n-1].ASSINCOME, name:'生产经营性收入户均（元）'},
                          {value:data.data.list[n-1].TRANINCOME, name:'转移性收入户均（元）'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].PROHOLDER,data.data.list[n-1].WORKHOLDER, data.data.list[n-1].ASSHOLDER, data.data.list[n-1].TRANHOLDER]
                  }
              ],
			 }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdinexclass",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdinexclass?"+b;
			console.log(cc);
