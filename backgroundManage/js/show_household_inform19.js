     ...
     var str =`
         <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">一般贫困人数 </td>						
					<td colspan="2">低保贫困户人数</td>						
					<td colspan="2">五保贫困人口</td>											
				</tr>		
                 <tr >						
					<td>人数</td>
					<td>占比（%）</td>
		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].A, name:'一般贫困人数'},
                          {value:data.data.list[n-1].B, name:'低保贫困户人数'},
                          {value:data.data.list[n-1].C, name:'五保贫困人口'}
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "povStatExcel/export/exist/povperstat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/povperstat?"+b;
			console.log(cc);
