  ...
  function   showDevelopment(index,obj,data){
     ...
     <table border="" cellspacing="" cellpadding="" style="width:1600px;overflow-x:auto">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">8个月</td>	
                    <td colspan="2">9个月</td>
                    <td colspan="2">10个月</td>						
					<td colspan="2">11个月</td>		
                    <td colspan="2">12个月</td>										
				</tr>		
                 <tr >						
					<td>人数</td>
					<td>占比（%）</td>
		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].B, name:'1个月'},
                          {value:data.data.list[n-1].C, name:'2个月'},
                          {value:data.data.list[n-1].D, name:'3个月'},
                          {value:data.data.list[n-1].E, name:'4个月'},
						  {value:data.data.list[n-1].F, name:'5个月'},
                          {value:data.data.list[n-1].G, name:'6个月'},
						  {value:data.data.list[n-1].H, name:'7个月'},
                          {value:data.data.list[n-1].I, name:'8个月'},
                          {value:data.data.list[n-1].J, name:'9个月'},
                          {value:data.data.list[n-1].K, name:'10个月'},
						  {value:data.data.list[n-1].L, name:'11个月'},
                          {value:data.data.list[n-1].M, name:'12个月'}
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 30,
                      data:[data.data.list[n-1].B, data.data.list[n-1].C, data.data.list[n-1].D, data.data.list[n-1].E, data.data.list[n-1].F,data.data.list[n-1].G, data.data.list[n-1].H, data.data.list[n-1].I, data.data.list[n-1].J, data.data.list[n-1].K,data.data.list[n-1].L, data.data.list[n-1].M]
                  }
              ],
			 }
	            ...
	            if(data.code==2000)
					{
                     ...
                     $("#total").html("总数:"+data.data.total);
				          ...
				          <td>人数</td>
							...
							<tr><td colspan='32' style="font-size:30px;color:#767272;">查无数据</td></tr>`
							seven.html(str)
							$("#total").empty();
						    $(".allPage").empty();
						}else if(data.code==1005){
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,seven,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "povStatExcel/export/exist/worktimestat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/worktimestat?"+b;
			console.log(cc);
