     ...
     var str =`
           <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">饮水困难户数</td>						
					<td colspan="2">无安全饮水户数</td>						
					<td colspan="2">未通生活用电户数</td>		
                    <td colspan="2">未通广播电视户数</td>						
					<td colspan="2">住房是危房户数</td>
                    <td colspan="2">无卫生厕所户数</td>						
					<td rowspan="2">人均住房面积（平方）</td>											
				</tr>		
                 <tr >						
					<td>户数</td>
					<td>占比（%）</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'饮水困难户数'},
                          {value:data.data.list[n-1].B, name:'无安全饮水户数'},
                          {value:data.data.list[n-1].C, name:'未通生活用电户数'},
                          {value:data.data.list[n-1].D, name:'未通广播电视户数'},
						  {value:data.data.list[n-1].E, name:'住房是危房户数'},
                          {value:data.data.list[n-1].F, name:'无卫生厕所户数'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdlife",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdlife?"+b;
			console.log(cc);
