  ...
  let token=$.cookie("token");
	...
	$.ajax({
		...
		success: function(data){
				if(data.code == 2000){
					showDevelopment(0,seventeen,data);
  ...
  function   showDevelopment(index,obj,data){
     ...
     <table border="" cellspacing="" cellpadding="" style="width:1600px;overflow-x:auto">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">1公里以内（不含）户数</td>						
					<td colspan="2">1（含）-3公里（不含）户数</td>						
					<td colspan="2">3（含）-5公里（不含）户数</td>		
                    <td colspan="2">5（含）-10公里（不含）户数</td>						
					<td colspan="2">10（含）-20公里（不含）户数</td>
                    <td colspan="2">20公里（含）以上户数</td>														
				</tr>		
                 <tr >						
					<td>户数</td>
					<td>占比（%）</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:13},
                      data:[
                          {value:data.data.list[n-1].A, name:'1公里以内户数'},
                          {value:data.data.list[n-1].B, name:'1-3公里户数'},
                          {value:data.data.list[n-1].C, name:'3-5公里户数'},
                          {value:data.data.list[n-1].D, name:'5-10公里户数'},
						  {value:data.data.list[n-1].E, name:'10-20公里户数'},
                          {value:data.data.list[n-1].F, name:'20公里以上户数'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
...
function page_init(index) {
	   ...
	   if( dataType == 'aar001'){
        ...
        $.ajax({
                ...
                if (data.code == 2000){
                          //  console.log("ok");
                     showDevelopment( index,seventeen,data); 
                        }
                },
                error:function () {
                }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdroaddist",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdroaddist?"+b;
			console.log(cc);
