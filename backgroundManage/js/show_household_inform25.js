$(document).ready(function(){
  ...
  let token=$.cookie("token");
	...
	$.ajax({
		url: url + "povStat/holderothreason",
  ...
  function   showDevelopment(index,obj,data){
     ...
     var str =`
	    	...
	    	<tr rowspan="2">
					...
					<td rowspan="2" valign="middle">贫困户数</td>
					<td colspan="2">因病致贫（户）</td>						
					<td colspan="2">因残致贫（户）</td>						
					<td colspan="2">因学致贫（户）</td>		
                    <td colspan="2">因灾致贫（户）</td>
					<td colspan="2">因婚致贫（户）</td>						
					<td colspan="2">缺土地（户）</td>
                    <td colspan="2">缺水（户）</td>						
					<td colspan="2">缺技术（户）</td>
                    <td colspan="2">缺劳力（户）</td>		
                    <td colspan="2">缺资金（户）</td>						
					<td colspan="2">交通条件落后（户）</td>
                    <td colspan="2">自身发展力不足（户）</td>						
					<td colspan="2">其他（户）</td>
				</tr>		
                 <tr >						
					<td>户数</td>
					<td>占比（%）</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:15},
                      data:[
                          {value:data.data.list[n-1].A, name:'因病致贫'},
                          {value:data.data.list[n-1].B, name:'因残致贫'},
                          {value:data.data.list[n-1].C, name:'因学致贫'},
                          {value:data.data.list[n-1].D, name:'因灾致贫'},
						  {value:data.data.list[n-1].E, name:'缺土地'},
                          {value:data.data.list[n-1].F, name:'缺水'},
						  {value:data.data.list[n-1].G,name:'缺技术'},
						  {value:data.data.list[n-1].H, name:'缺劳力'},
                          {value:data.data.list[n-1].I, name:'缺资金'},
                          {value:data.data.list[n-1].J, name:'交通条件落后'},
						  {value:data.data.list[n-1].K, name:'自身发展力不足'},
                          {value:data.data.list[n-1].M, name:'因婚致贫'},
						  {value:data.data.list[n-1].L,name:'其他'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 26,
                      data:[data.data.list[n-1].A,data.data.list[n-1].B, data.data.list[n-1].C, data.data.list[n-1].D,data.data.list[n-1].E,data.data.list[n-1].F,data.data.list[n-1].G,data.data.list[n-1].H, data.data.list[n-1].I, data.data.list[n-1].J,data.data.list[n-1].K,data.data.list[n-1].M,data.data.list[n-1].L]
                  }
              ],
			 }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holderothreason",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holderothreason?"+b;
			console.log(cc);
