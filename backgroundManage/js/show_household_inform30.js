     ...
     var str =`
          <table border="" cellspacing="" cellpadding="">		   
	    		<tr rowspan="2">
					...
					<td colspan="2">柴草户数 </td>						
					<td colspan="2">干畜粪户数</td>						
					<td colspan="2">煤炭户数</td>	
                    <td colspan="2">清洁能源户数 </td>						
					<td colspan="2">其他</td>															
				</tr>		
                 <tr >						
					<td>户数</td>
	    	   ...
	    	   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].A, name:'柴草户数'},
                          {value:data.data.list[n-1].B, name:'干畜粪户数'},
						  {value:data.data.list[n-1].C, name:'煤炭户数'},
                          {value:data.data.list[n-1].D, name:'清洁能源户数'},
                          {value:data.data.list[n-1].E, name:'其他'}
                      ],
                      itemStyle: {
                          emphasis: {
                              shadowBlur: 10,
				 ...
				 series : [
                  {
                      ...
                      type:'bar',
                      barWidth : 26,
                      data:[data.data.list[n-1].A,data.data.list[n-1].B, data.data.list[n-1].C, data.data.list[n-1].D,data.data.list[n-1].E]
                  }
              ],
			 }
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "holderStatExcel/export/exist/holdfuel",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"holderStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"holderStatExcel/export/holdfuel?"+b;
			console.log(cc);
