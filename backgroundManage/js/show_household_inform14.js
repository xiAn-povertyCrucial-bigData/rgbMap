		   ...
		   series : [
                  {
                      ...
                      label:{fontSize:16},
                      data:[
                          {value:data.data.list[n-1].A, name:'文盲半文盲'},
                          {value:data.data.list[n-1].B, name:'小学'},
                          {value:data.data.list[n-1].C, name:'初中'},
                          {value:data.data.list[n-1].D, name:'高中'},
						  {value:data.data.list[n-1].E, name:'大专及以上'}
...
$("body").on("click","#dataImport",function(){
		$.ajax({
        url: url + "povStatExcel/export/exist/edustat",  
        data:{},
        beforeSend:function(request){
            request.setRequestHeader("Authorization",token);
            request.setRequestHeader("X-Requested-With","XMLHttpRequest");
        ...
        if (data.code == 2000){
         ...
         //   let formList=$("#formSubmit2");
           // let urls=url+"povStatExcel/export/perbasicstat";
			...
			console.log(b);
			let cc=url+"povStatExcel/export/edustat?"+b;
			console.log(cc);
