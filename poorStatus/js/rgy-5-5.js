...
$("body").on("click","#c-title1",function(){
	$('#zhezhao').show();
   	str = `
    <h3 class="c-title">周至县各街道贫困户五项退出指标达标情况统计<span id="content-4shi-close" class="content-4shi-close" style="float:right;color: white;padding-right: 10px;">X</span>
...
$("body").on("click",".content4-1-shi span",function(){
     ...
     var codeType =  $(this).attr("sleep");
   	str1 = `
    <h3 class="c-title"><span id='zhennames'></span>各村贫困户五项退出指标达标情况统计<span id="content-4zhen-close" class="content-4zhen-close" style="float:right;color: white;padding-right: 10px;">X</span>
    <a class="content-4zhen-return" id='content-4zhen-return'></a>
    </h3>`;
  $.ajax({
            url: url+'map/five',
            data:{'year':y,'disctId':codeType},
            method: "post",
            dataType: "json",
            success: function(data){
                if (data.code == 2000){
