        ...
        $.ajax({
            ...
            success:function (result) {
               ...
               for(let i = 0; i < data.length; i++){
                 str +=`  	<li>
                        <div style="width:30.33%;">${data[i].PERSON_NAME}</div>
                        <div style="width:33.33%;">${data[i].LOAN_MONEY}</div>
                        <div style="width:36.33%;">${data[i].LOAN_PURPOSE}</div>
                    </li>
                    `;
               }
...
//         }else if(param.dataIndex==1){
        ...
        //集中安置 村区
            $.ajax({
                ...
                method:"post",
                dataType:"json",
                data:{'cityNo':disctId1,'placement':'0'},
                success:function (result) {
                    if(result.code==2000){
                        for(let i = 0; i < result.data.length; i++ ){
                            n1[i] = result.data[i].AAA011;
