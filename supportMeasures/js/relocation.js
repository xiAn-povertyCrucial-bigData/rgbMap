$(document).ready(function() {
    ...
    function industryPoverty() {
        let pieRadius = ['0', '50%'];
        let yHeight = '28%';
        // let dom2 = document.getElementById("module-1");
        // let myChart2 = echarts.init(dom2);
        $.ajax({
            url: url + "/place/reloc/query",
            method: "get",
            dataType: "json",
            data: {},
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (let i = 0; i < data[0].length; i++) {
                    datas2[i] = data[0][i].no;
                    datas3[i] = data[0][i].ongoing;
                    datas4[i] = data[0][i].reach;
                    datas5[i] = data[0][i].AAR008;
                    datas[i] = data[0][i].AAR009;
                }
                //    alert(datas);
                option = {
                    color: ['#DD4F43', '#FFCE43', '#1FA463'],
    ...
    function eConsole4(param) {
        ...
        if (1) {
            ...
            myChart24.on("click", function (params) {
                ...
                console.log(params)
                if (params.componentType == "xAxis") {
                    $("#countyName12").text(params.value);
